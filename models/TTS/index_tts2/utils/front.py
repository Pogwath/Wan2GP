...
class TextNormalizer:
    def __init__(self, enable_glossary=False):
        ...
        self.zh_char_rep_map_pattern = re.compile("|".join(re.escape(p) for p in self.zh_char_rep_map.keys()))
        self.char_rep_map_pattern = re.compile("|".join(re.escape(p) for p in self.char_rep_map.keys()))
        ...
        self.origin_pinyin_pattern = re.compile(TextNormalizer.PINYIN_TONE_PATTERN, re.IGNORECASE)
    ...
    def normalize(self, text: str) -> str:
        ...
        if self.use_chinese(text):
            ...
            result = self.zh_char_rep_map_pattern.sub(lambda x: self.zh_char_rep_map[x.group()], result)
        else:
            try:
                text = re.sub(TextNormalizer.ENGLISH_CONTRACTION_PATTERN, r"\1 is", text, flags=re.IGNORECASE)
                # 应用术语词汇表（优先级最高，在所有保护之前）
                ...
                result = text
                print(traceback.format_exc())
            result = self.char_rep_map_pattern.sub(lambda x: self.char_rep_map[x.group()], result)
        return result
    ...
    def save_names(self, original_text):
        ...
        """
        # 人名
        original_name_list = re.findall(self.name_pattern, original_text)
    ...
    def save_pinyin_tones(self, original_text):
        ...
        """
        # 声母韵母+声调数字
        original_pinyin_list = re.findall(self.origin_pinyin_pattern, original_text)
